...
monitoring dashboard (\S\ref{sec:current:square:squash}).  Note that \code{lsst.verify}
...
restricted to only KPMs. Any metric defined in \code{lsst.verify.metrics} can
...
port to the \code{lsst.verify} system on the \code{verify\_port} branch.
...
System Requirements (\citeds{LSE-61}) should be updated to reflect this.
